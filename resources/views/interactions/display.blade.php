    ...
    <div class="container ">
        <div class="p-3 " style=" border-style: outset; border-radius: 6px">
            <form method="POST" action="/newinteractions">
                ...
                <script>
                    function answer(response, cp) {
                        if (response) {
                            document.querySelector('#Cp' + cp + '_1').style.display = "block";
                            document.querySelector('#Cp' + cp + '_2').style.display = "none";
                        } else {
                            document.querySelector('#Cp' + cp + '_1').style.display = "none";
                            document.querySelector('#Cp' + cp + '_2').style.display = "block";
                        }
                    }
                </script>
                <div class="mb-3 mt-3">
                    <p>Bonjour {{ $volunteer->first_name }} {{ $volunteer->last_name }}, je m’appelle
                        {{ auth()->user()->username }} je suis bénévole pour AG!SSONS.
                        Je vous contacte parce que vous avez signé l’appel pour des transports publics très abordables et
                        accessibles.
                        Je l’ai signé moi aussi, et aujourd’hui avec d’autres bénévoles, j’appelle d’autres sympathisants
                        pour discuter de cette campagne pour les transports
                        publics et contre les extensions d’autoroutes pour voir comment on peut gagner ensemble.
                    </p>
                    <button type="button" id='Answer' onclick="answer(true,'1')" class="btn btn-primary col-5"
                        style="background-color: #a7c957;color:#bc4749;border:#386641">Oui</button>

                    <button type="button" id='noAnswer' onclick="answer(false,'1')" class="btn btn-primary col-5"
                        style="background-color: #a7c957;color:#bc4749;border:#386641">Non</button>

                </div>

                <div id ='Cp1_1' style="display: none">
                    <div class="mb-3 mt-3">
                        <p>
                            avez vous le temps de discuter
                        </p>
                        <button type="button" id='Answer' onclick="answer(true,'2')" class="btn btn-primary col-5"
                            style="background-color: #a7c957;color:#bc4749;border:#386641">Oui</button>

                        <button type="button" id='noAnswer' onclick="answer(false,'2')" class="btn btn-primary col-5"
                            style="background-color: #a7c957;color:#bc4749;border:#386641">Non</button>
                    </div>
                    <div id ='Cp2_1' style="display: none">
                        <div class="mb-3 mt-3">
                            <p>
                                seriez vous interessé par du phoning
                            </p>
                            <button type="button" id='Answer' onclick="answer(true,'3')" class="btn btn-primary col-5"
                                style="background-color: #a7c957;color:#bc4749;border:#386641">Oui</button>

                            <button type="button" id='noAnswer' onclick="answer(true,'4')" class="btn btn-primary col-5"
                                style="background-color: #a7c957;color:#bc4749;border:#386641">Non</button>

                        </div>
                        <div id ='Cp3_1' style="display: none">
                            <div class="mb-3 mt-3">
                                <p>
                                    voilà nos differente sceances laquelle vous botterais
                                </p>
                                <button type="button" id='Answer' onclick="answer(true,'3')"
                                    class="btn btn-primary col-5"
                                    style="background-color: #a7c957;color:#bc4749;border:#386641">Oui</button>

                                <button type="button" id='noAnswer' onclick="answer(false,'3')"
                                    class="btn btn-primary col-5"
                                    style="background-color: #a7c957;color:#bc4749;border:#386641">Non</button>
                    ...
                    <div id ='Cp2_2' style="display: none">
                        <div class="mb-3 mt-3">
                            <p>
                                quand pourais-je vous rappeler
                            </p>
                            <label for="next_date">date de la prochaine action:</label>
                            <input type="datetime-local" class="form-control" name="next_date"
                                placeholder="date de la prochaine action" value="{{ old('next_date') }}">
                            @error('next_date')
                                <p>{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div id ='Cp1_2' style="display: none">
                    <div class="mb-3 mt-3">
                        <p>
                            envoyer un message
                        </p>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" class="form-control" name="next_move"
                        placeholder="Decrivez la prochaine action" value="{{ old('next_move') }}">
                    @error('next_move')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="number" class="form-control" name="donation_pledge"
                        placeholder="Montant de la promesse de don" value="{{ old('donation_pledge') }}">
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" class="form-control" name="entousiasmed"
                        placeholder="Decrivez la prochaine action" value="{{ old('entousiasmed') }}">
                    @error('entousiasmed')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" class="form-control" name="engagement_spe"
                        placeholder="Decrivez la prochaine action" value="{{ old('engagement_spe') }}">
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" class="form-control" name="ressource"
                        placeholder="Decrivez la prochaine action" value="{{ old('ressource') }}">
                    @error('ressource')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
            ...
            <form method="POST" action="/newinteractions" style="display:none">
                @csrf <!-- juste de la securité  https://laravel.com/docs/5.8/csrf -->
                <div class="mb-3 mt-3" hidden>
                    <label for="volunteer_id">contenu de l'interaction</label>
                    <input type="number" class="form-control" name="volunteer_id"
                        placeholder="contenu de l'interaction" value="{{ $volunteer->id }}">
                    @error('volunteer_id')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-3 mt-3" hidden>
                    <label for="interactor_id">contenu de l'interaction</label>
                    <input type="number" class="form-control" name="interactor_id"
                        placeholder="contenu de l'interaction" value="{{ auth()->user()->id }}">
                    @error('interactor_id')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
